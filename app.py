...
genai.configure(api_key="AIzaSyCNgue8VnElNvumro_nD9U_GMfxyL1D9Gc")  # ⚠️ Replace with your actual API key
...
def chat():
    try:
        ...
        if not any(topic in user_input for topic in ALLOWED_TOPICS):
            response_text = "I'm here to discuss community clean-up and sustainability. Try asking about recycling, volunteering, or reducing waste!"
        else:
        ...
        if len(chat_sessions[session_id]['messages']) <= 2:  # User + bot messages
            chat_sessions[session_id]['title'] = user_input[:30] + ('...' if len(user_input) > 30 else '')
...
def get_sessions():
    try:
        # Return all chat sessions sorted by creation date (newest first)
        sessions = [
            {'id': id, 'title': data['title'], 'created_at': data['created_at']}
...
def new_session():
    try:
        ...
        chat_sessions[session_id] = {
            ...
            'messages': [{
                'sender': 'bot',
                'message': "Hello! I'm here to help with community clean-up initiatives. How can I help you today?",
                'timestamp': datetime.now().isoformat()
            }]
        }
        return jsonify({'session_id': session_id})
    except Exception as e:
        app.logger.error(f"Error creating new session: {str(e)}")
        return jsonify({'error': str(e)}), 500
